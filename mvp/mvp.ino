...
delay(100);
  while (1) {
    ...
    {
      lcd.setCursor(0, 0);
      lcd.print(F("SPI interface Error!"));
      delay(1000); continue;
    } else {
      lcd.setCursor(0, 0);
      lcd.print(F("SPI interface OK."));   delay(250); break;
    }
  }
#if defined (OV2640_MINI_2MP_PLUS)
  while (1) {
    ...
    if ((vid != 0x26 ) && (( pid != 0x41 ) || ( pid != 0x42 ))) {
      lcd.setCursor(0, 0);
      lcd.print(F("ACK CMD Can't find OV2640 module!"));
      delay(1000); continue;
    }
    else {
      lcd.setCursor(0, 0);
      lcd.print(F("ACK CMD OV2640 detected.")); break;
    }
  }
#endif
  //Initialize SD Card
  ...
  {
    lcd.setCursor(0, 0);
    lcd.println(F("SD Card Error!")); delay(1000);
  }
  lcd.setCursor(0, 0);
  lcd.println(F("SD Card detected."));
  delay(250);
...
void loop() {
  ...
  myCAM.start_capture();
  lcd.setCursor(0, 0);
  lcd.print(F("start capture."));
  delay(250);
  total_time = millis();
  while ( !myCAM.get_bit(ARDUCHIP_TRIG, CAP_DONE_MASK));
  lcd.setCursor(0, 0);
  lcd.println(F("Capture Done.   "));
  delay(250);
